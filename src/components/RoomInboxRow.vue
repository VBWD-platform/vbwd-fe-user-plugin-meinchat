  ...
  >
    ...
    <div class="inbox-row__main">
      <div class="inbox-row__top">
        <strong class="inbox-row__nick">{{ room.name || '#' }}</strong>
        <span
...
const formattedTime = computed(() => {
  if (!props.room.last_message_at) return '';
  const sent = new Date(props.room.last_message_at);
  const now = new Date();
